...
void InitMyMesh() {
    ...
    try {
        // 정육면체의 각 면을 사각형으로 추가
        mesh.add_quad(v0, v1, v2, v3); // 아래 면
        mesh.add_quad(v4, v5, v6, v7); // 위 면
        mesh.add_quad(v0, v1, v5, v4); // 앞 면
        mesh.add_quad(v2, v3, v7, v6); // 뒤 면
        mesh.add_quad(v0, v3, v7, v4); // 왼쪽 면
        mesh.add_quad(v1, v2, v6, v5); // 오른쪽 면
